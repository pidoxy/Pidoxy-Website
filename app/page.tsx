...
import { ArrowRight, FileText, Download, Github, Linkedin, Mail, Code, Terminal } from "lucide-react";
        ...
        <section id="about" className="mb-24 pt-10">
          <div className="max-w-3xl">
            ...
            <div className="flex flex-wrap gap-3 mb-10">
              ...
              <a href="/resume_swe.pdf" target="_blank" className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-slate-900 text-white font-medium text-sm hover:bg-slate-700 transition-colors shadow-sm">
                <Terminal size={16} /> Engineering CV
              </a>
              <a href="/resume_research.pdf" target="_blank" className="flex items-center gap-2 px-4 py-2.5 rounded-full bg-white border border-slate-200 text-slate-700 font-medium text-sm hover:border-blue-300 hover:text-blue-600 transition-colors shadow-sm">
            ...
            {/* NEW: SKILLS PILLS */}
            <div>
              <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3">Core Technologies</p>
              <div className="flex flex-wrap gap-2">
                {profileData.skills.map((skill) => (
                  <span key={skill} className="px-3 py-1.5 text-sm font-medium text-slate-700 bg-slate-100 rounded-md border border-slate-200">
                    {skill}
                  </span>
                ))}
              </div>
           ...
           <div className="relative border-l-2 border-slate-100 ml-3 space-y-12">
            {experienceData.map((role, index) => (
              <div key={index} className="relative pl-8">
                <div className={`absolute -left-[9px] top-2 h-4 w-4 rounded-full border-2 border-white shadow-sm ${
